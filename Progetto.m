%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%PROGETTO MATLAB 2020 MECCANICA APPLICATA%%%%
clear
close all
clc
...
%Parametri al variare dal C.P. 10700497 
%L27=300+10*x+y 
%L2G=0.4*L27  
x=9;
y=7;
...
%SISTEMA
%c*cos(p)+a*cos(alpha)=b*cos(beta);
%c*sin(p)+a*sin(alpha)=b*sin(beta);
%ANGOLI E GEOMETRIA
...
alphapp=Y(1);
betapp=Y(2);
disp(['Accelerazione angolare dell asta L24 è: ',num2str(alphapp),'  rad/s^2']);
disp(['Accelerazione angolare dell asta L45 è: ',num2str(betapp),'   rad/s^2']);
fprintf('Segno negativo: velocità in senso orario\n\n\n');
%%
...
%Cm*omega+Mav*(G*VG.')+Mma*(G*VM.') => Sommatoria potenze 
%d(1/2Mav*VG^2+1/2Jav*deltap^2+1/2Mma*VM^2)/dt =>Derivata energia cinetica
...
title('FUNZIONE DI TRSAFERIMENTO');
ylabel('MODULO FDT');
